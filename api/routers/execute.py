...
async def execute_stream(
    ...
    # Dev bypass check
    dev_bypass = os.getenv("REGISTRY_BYPASS_DEV", "").strip().lower() in ("1", "true", "yes")

    if not payment_header and not tx_header and not dev_bypass:
    ...
    async def event_generator():
        ...
        if dev_bypass:
             yield f"data: {json.dumps({'line': '[WARN] REGISTRY_BYPASS_DEV active — skipping payment verification.'})}\n\n"
        elif use_facilitator:
            push_event(
                kind="execute",
                severity="info",
        ...
        elif not registry_client.contract_id:
